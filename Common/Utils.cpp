...
unsigned int	OpenAndReadFile(const char* FileName, char** Buffer)
{
	...
	if (hFile == -1) {
		if (errno == ETXTBSY) {
			IO_TRACE("_open failed ETXTBSY %s %d\n", FileName, errno);
			int iTry = 0;
			while (++iTry < 20) {
				Sleep(10);
		...
		if (hFile == -1)
		{
			IO_TRACE("_open failed %s error = %d\n", FileName, errno);
			return 0;
		}
	}
	...
	Size = _lseek(hFile, 0, SEEK_END);
	_lseek(hFile, 0, SEEK_SET);
	if (*Buffer == 0)
		*Buffer = new char[Size];
	read_ = _read(hFile, (LPVOID)*Buffer, Size);
	if (Size != read_)
	{
		if (EAGAIN == errno)
			read_ = _read(hFile, (LPVOID)Buffer, Size);
...
int		OpenFile(const char* FileName, bool create, bool shared, bool write_mode)
{
	...
	if (create) 	{
		flag |= O_CREAT;
		hFile = _open(FileName, flag, S_IREAD | S_IWRITE);
	}
	else hFile = _open(FileName, flag);

	if (hFile == 0)
		hFile = _open(FileName, flag);
	if (hFile == -1)	{
		if ( errno == ETXTBSY )	{
			IO_TRACE("_open failed ETXTBSY %s %d\n", FileName, errno);
			int iTry = 0;
...
bool FileWrite(int hFile, void* Buffer, unsigned int Size)
{
	...
	read_ = _write(hFile, Buffer, Size);
	if (Size != read_)	{
