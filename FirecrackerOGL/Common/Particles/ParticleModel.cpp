...
void CFireworkParticleModel::InitParticle(CParticle& Particle, float Vx, float Vy, float Vz, std::function< float(float) > distributionFunction) {
}
...
void CModelGravitation::InitParticle( CParticle& particle, float Vx, float Vy, float Vz, std::function< float(float) > distributionFunction)
{
...
void CModelGravitation::CaclParticle(CParticle& particle, float Dt)
{
	...
	if (Particle.life > 100.0f || Particle.life < 0.1f) { //  частица улетела или сгорела
		InitParticle(Particle, m_fXspeed, m_fYspeed, 0.0f, [](float x) { return x; });
	}
}



void CModelStream::InitParticle(CParticle& particle, float Vx, float Vy, float Vz, std::function< float(float) > distributionFunction)
{
	CGravityParticle& Particle = dynamic_cast<CGravityParticle&>(particle);
	static auto num = 0;
	Particle.setActive( true );									// Make All The Particles Active
